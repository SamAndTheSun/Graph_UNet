...
def make_npy(dataset_dir, 
    ...
    param dataset_dir: the path containing the processed UKBB files, str
    ...
    # account for inconsistent order of month and day in folder names

    df_yyxx = metadata_csv.copy()  # will store ZZZZYYXX format
    df_xxyy = metadata_csv.copy()  # will store ZZZZXXYY format

    df_yyxx[date_column] = df_yyxx[date_column].apply(
    lambda x: (
        f"20{x.split('/')[2].zfill(2)}"  # year
        f"{x.split('/')[0].zfill(2)}"    # month 
        f"{x.split('/')[1].zfill(2)}"     # day
        ) if pd.notna(x) else None
    )

    df_xxyy[date_column] = df_xxyy[date_column].apply(
        lambda x: (
            f"20{x.split('/')[2].zfill(2)}"  # year 
            f"{x.split('/')[1].zfill(2)}"    # day (swapped)
            f"{x.split('/')[0].zfill(2)}"     # month (swapped)
        ) if pd.notna(x) else None
    )

    metadata_csv = pd.concat([df_yyxx, df_xxyy], ignore_index=True)

    # remove non-CNs
    subjects_without_all_files.extend(metadata_csv[metadata_csv['DXCURREN'] != 1].apply(lambda row: f"{row[id_column]}_{row[date_column]}", axis=1).tolist())
...
def get_X_and_y(num_nodes, subject_ids, date_column, 
    ...
    for subj_idx, subject_id in enumerate(subject_ids):
        ...
        except (IndexError, TypeError, ValueError): # if there isn't an associated age value, we don't consider this subject
            continue
...
folder = f'AIBL_ico{ico}_all/'
raw_data_dir = f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/raw/{folder}'
"""
damaged_subjects = make_data(root_path='/mnt/md0/tempFolder/', file_types=file_types, # used to be ratio so older sorts are incorrect
             data_path='samAnderson/datasets/AIBL/*/freesurfer_output/*/',
...
X, y, damaged_subjects = make_npy(dataset_dir=raw_data_dir, 
              metadata_path='/mnt/md0/tempFolder/samAnderson/datasets/AIBL_master.csv',
              file_suffixes = file_suffixes,
              age_id_date=['AGE', 'subj_id', 'MRIEXAMDATE'],
              massive=False,
              first_hemi='rh')
...
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/X_AIBL_CN', X_norm)
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/y_AIBL_CN', y)
