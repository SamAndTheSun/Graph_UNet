...
def validate_inputs(input_path, file_type):
    ...
    file_type_ranges = {
        'area': {'min': -1, 'max': 100}, # made lower than normal for ADNI due to high exclusion rate
        'curv': {'min': -40, 'max': 40},
    ...
    try:
        ...
        # For surface geometry files
            ...
            verts, _ = nib.freesurfer.read_geometry(input_path)
...
def make_npy(dataset_dir, 
    ...
    metadata_csv[date_column] = pd.to_datetime(metadata_csv[date_column], dayfirst=True).dt.strftime('%Y%m%d')
    ...
    # remove Men
    subjects_without_all_files.extend(metadata_csv[metadata_csv['Sex'] != 'M'].apply(lambda row: f"{row[id_column]}_{row[date_column]}", axis=1).tolist())
...
X, y, damaged_subjects = make_npy(dataset_dir=raw_data_dir, 
              ...
              age_id_date=['Age', 'Subject ID', 'Study Date'],
              massive=False,
              first_hemi='rh')
...
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/X_ADNI_CN_M', X_norm)
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/y_ADNI_CN_M', y)