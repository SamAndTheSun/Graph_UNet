...
def make_fsaverage(input_path, output_path, file_type, ico=7):
    ...
    if 'w-g.pct.mgh' in input_path:
        output_fname_abs = f'{output_path}{input_path.split("/")[subj_dir]}_{hemi[-2:]}.w-g.pct.mgh'
    else:
        output_fname_abs = f'{output_path}{input_path.split("/")[subj_dir]}_{hemi[-2:]}_{".".join(input_path.split(".")[-1:])}.mgh'
...
def get_X_and_y(num_nodes, subject_ids, date_column, 
    ...
    for subj_idx, subject_id in enumerate(subject_ids):
        ...
        target_id = subject_id.rsplit('_', 1)[0]
...
folder = f'AOMIC_ID1000_ico{ico}_all'
...
damaged_subjects = make_data(root_path='/mnt/md0/tempFolder/samAnderson/datasets/', file_types=file_types, # used to be ratio so older sorts are incorrect
             data_path='AOMIC/AOMIC_ID1000/derivatives/freesurfer/sub-*/',
...
X, y, damaged_subjects = make_npy(dataset_dir=raw_data_dir, 
              ...
              age_id_date=['age', 'participant_id', None],
              massive=False,
              first_hemi='rh')
...
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/X_AOMIC_ID1000', X_norm)
np.save(f'/mnt/md0/tempFolder/samAnderson/gnn_model/unet-gnn/datasets/processed/y_AOMIC_ID1000', y)